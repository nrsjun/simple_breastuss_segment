...
# Testing the image processing classes
if __name__ == "__main__":
    print("Testing Image Processing Classes...")
    ...
    is_valid = ImageProcessor.validate_image(test_image)
    ...
    # Test normalization
    normalized = ImageProcessor.normalize_image(test_image)
    print(f"Normalized image range: {normalized.min():.2f} - {normalized.max():.2f}")
    
    # Test ResultProcessor
    print("\\n--- Testing ResultProcessor ---")
    
    # Create test mask
    test_mask = np.zeros((300, 400), dtype=np.uint8)
    cv2.circle(test_mask, (200, 150), 50, 1, -1)  # Create circular tumor
    
    # Test tumor metrics
    metrics = ResultProcessor.calculate_tumor_metrics(test_mask)
    print(f"Tumor metrics: {metrics}")
    
    # Test tumor detection
    detected = ResultProcessor.detect_tumor_presence(test_mask)
    print(f"Tumor detected: {detected}")
    
    # Test overlay generation
    overlay = ResultProcessor.generate_overlay(test_image, test_mask)
    ...
    seg_result = {
        'tumor_detected': True,
        'tumor_area_pixels': 1200,
        'success': True
    }
    
    formatted = ResultProcessor.format_results(cls_result, seg_result)
    print(f"Formatted results keys: {list(formatted.keys())}")
    print(f"Summary: {formatted['summary']['risk_level']} risk, {len(formatted['summary']['recommendations'])} recommendations")
    
    print("\\nImage processing classes work correctly!")